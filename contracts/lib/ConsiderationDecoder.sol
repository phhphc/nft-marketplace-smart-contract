...
contract ConsiderationDecoder {
    ...
    function _decodeBytes(
        ...
        assembly {
            ...
            let size := add(
                and(add(calldataload(cdPtrLength), ThirtyOneBytes), OnlyFullWordMask),
                OneWord
            )
        ...
        returns (uint256 invalidEncoding, MemoryPointer offer, MemoryPointer consideration)
    {
        assembly {
            ...
            function copySpentItemsAsOfferItems(rdPtrHead, length) -> mPtrLength {
                ...
                mstore(
                    FreeMemoryPointerSlot,
                    add(mPtrLength, add(OneWord, mul(length, OfferItem_size_with_length)))
                )
    ...
    function _convertGetGeneratedOrderResult(
        function() internal pure returns (uint256, MemoryPointer, MemoryPointer) inFn
    )
        internal
        pure
        returns (
            function()
                internal
                pure
                returns (uint256, OfferItem[] memory, ConsiderationItem[] memory) outFn
        )
    {
        assembly {
            outFn := inFn
    ...
    ) internal pure returns (function(OfferItem memory, address) internal outFn) {
        assembly {
            outFn := inFn
        }
    }
    ...
    function _toConsiderationItemInput(
        function(ReceivedItem memory, address) internal inFn
    ) internal pure returns (function(ConsiderationItem memory, address) internal outFn) {
        assembly {
            outFn := inFn
        }
    }
    ...
    function _toOrderParametersReturnType(
        ...
        internal
        pure
        returns (function(CalldataPointer) internal pure returns (OrderParameters memory) outFn)
    {
        assembly {
            outFn := inFn
        }
    ...
    function _toAdvancedOrderReturnType(
        ...
        internal
        pure
        returns (function(CalldataPointer) internal pure returns (AdvancedOrder memory) outFn)
    {
        assembly {
            outFn := inFn
        }
    ...
    function _toOrdersReturnType(
        ...
        internal
        pure
        returns (function(CalldataPointer) internal pure returns (Order[] memory) outFn)
    {
        assembly {
            outFn := inFn
        }
