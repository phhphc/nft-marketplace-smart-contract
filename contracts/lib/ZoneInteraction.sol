...
import {OrderType} from "./TraderEnums.sol";

import {AdvancedOrder, OrderParameters, Order} from "./TraderStructs.sol";
...
contract ZoneInteraction is ConsiderationEncoder, ZoneInteractionErrors, LowLevelHelpers {
     ...
     */
    function _assertRestrictedOrderValidity(
        ...
        if (_isRestrictedAndCallerNotZone(parameters.orderType, parameters.zone)) {
            ...
            (callData, size) = _encodeValidateOrder(orderHash, parameters, orderHashes);

            // Set the target to the zone.
            target = (
                parameters.toMemoryPointer().offset(OrderParameters_zone_offset).readAddress()
            );
