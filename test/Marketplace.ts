    ...
    describe("Trader", function () {
        ...
        describe("Listing", function () {
            ...
            it("should emit listing event", async function () {
                ...
                await expect(
                    ...
                    .to.emit(marketplace, "Listing")
                    .withArgs(myToken.address, tokenIds[0], listingPrice);
            });

            it("should add token to listing", async function () {
                const { marketplace, owner } = await loadFixture(deployMarketplaceFixture);
                const { myToken, tokenIds } = await loadFixture(deployErc721WithTokensFixture);
                ...
                const listingItem = await marketplace.getListing(myToken.address, tokenIds[0]);
        ...
        describe("Cancel Listing", function () {
            it("should emit listing canceled event", async function () {
                const { marketplace, myToken, tokenId } = await loadFixture(deployMarketplaceWithListingFixture);

                await expect(marketplace.cancelListing(myToken.address, tokenId))
                    .to.emit(marketplace, "ListingCanceled")
                    .withArgs(myToken.address, tokenId);
            });

            it("should revert on non-owner sender", async function () {
                const { marketplace, myToken, tokenId, otherAccount } = await loadFixture(
                    deployMarketplaceWithListingFixture,
                );

                await expect(
                    marketplace.connect(otherAccount).cancelListing(myToken.address, tokenId),
                ).to.be.revertedWith("You aren't the seller");
            });

            it("should return token to owner", async function () {
                const { marketplace, myToken, tokenId, otherAccount, owner } = await loadFixture(
                    deployMarketplaceWithListingFixture,
                );

                await marketplace.cancelListing(myToken.address, tokenId);
            ...
            it("should remove token from listing", async function () {
                ...
                await marketplace.cancelListing(myToken.address, tokenId);
                await expect(marketplace.getListing(myToken.address, tokenId)).to.be.revertedWith(
                    "Listing doesn't exists",
                );
            });

            it("should revert if token is not listing", async function () {
                const { marketplace, myToken, tokenId, listingPrice, otherAccount, owner } = await loadFixture(
                    deployMarketplaceWithListingFixture,
                );

                await expect(marketplace.cancelListing(myToken.address, tokenId + 1)).to.be.revertedWith(
                    "Listing doesn't exists",
                );
            });
        });
        ...
        describe("Buy", function () {
            it("should emit sale event", async function () {
                ...
                await expect(marketplace.connect(otherAccount).buy(myToken.address, tokenId, { value: listingPrice }))
                    .to.emit(marketplace, "Sale")
                    .withArgs(myToken.address, tokenId, owner.address, otherAccount.address, listingPrice);
            });
            ...
            it("should transfer token to buyer", async function () {
                ...
                await marketplace.connect(otherAccount).buy(myToken.address, tokenId, { value: listingPrice });
            ...
            it("should transfer ether to token owner", async function () {
                ...
                await expect(
                    marketplace.connect(otherAccount).buy(myToken.address, tokenId, { value: listingPrice }),
            ...
            it("should revert on incorrect token value", async function () {
                ...
                await expect(
                    marketplace.connect(otherAccount).buy(myToken.address, tokenId, { value: listingPrice.add(1) }),
                ).to.be.revertedWith("You didn't provide the correct price");

                await expect(
                    marketplace.connect(otherAccount).buy(myToken.address, tokenId, { value: listingPrice.sub(1) }),
                ).to.be.revertedWith("You didn't provide the correct price");
            });
            ...
            it("should revert if token is not listing", async function () {
                ...
                await expect(
                    marketplace.connect(otherAccount).buy(myToken.address, tokenId + 1, { value: listingPrice }),
